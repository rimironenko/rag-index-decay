...
inspectable, not to ship the checks.

You don't have to trust the private code to check the findings, though.
The churn replay rebuilds the full ground-truth ledger right next to the
index: `index_events` records which chunks should be present in which
engine after every operation, and `erasure_requests` records what should
be gone after the synthetic-PII erasure step. So verifying any number in
`results/t2_audit.json` is just a diff - what the ledger says should be
there vs. what the engine actually returns. You can refute the findings
straight from the ledger without ever seeing the code that produced them.
...
produced it. You can still check those numbers, though: the ledger this
replay rebuilds carries the expected state of every chunk, so the
findings are diffable against ground truth without the check code (see
"What's published here vs. what isn't" above).
